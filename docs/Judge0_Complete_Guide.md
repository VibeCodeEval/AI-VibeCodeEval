...
Worker는 Redis Queue를 사용하므로 Redis 서버가 실행 중이어야 합니다.
...
# 큐 설정
USE_REDIS_QUEUE=True  # Redis Queue 사용 (권장)
```
...
```
2025-12-05 12:00:00,000 - app.application.workers.judge_worker - INFO - [JudgeWorker] Worker 시작
2025-12-05 12:00:00,100 - app.application.workers.judge_worker - INFO - [JudgeWorker] Redis 연결 완료
```

**작업 처리 시:**
```
2025-12-05 12:00:05,000 - app.application.workers.judge_worker - INFO - [JudgeWorker] 작업 처리 시작 - task_id: correct_web-session-...
2025-12-05 12:00:10,000 - app.application.workers.judge_worker - INFO - [JudgeWorker] 작업 완료 - task_id: ..., status: success, time: 0.01s
```

### Worker 중지

**Ctrl + C**를 눌러 Worker를 중지할 수 있습니다.

```
^C
2025-12-05 12:00:15,000 - app.application.workers.judge_worker - INFO - [JudgeWorker] Worker 중지 요청 수신
2025-12-05 12:00:15,100 - app.application.workers.judge_worker - INFO - [JudgeWorker] Redis 연결 종료
2025-12-05 12:00:15,200 - app.application.workers.judge_worker - INFO - [JudgeWorker] Worker 중지
```

### 여러 Worker 실행 (확장)

여러 Worker를 실행하여 병렬 처리 가능:
...
각 Worker가 큐에서 작업을 가져와 병렬로 처리합니다.
...
2. **Judge0 Worker 실행** (새 터미널)
   ```bash
   uv run python -m app.application.workers.judge_worker
   ```

3. **FastAPI 서버 실행** (새 터미널)
   ```bash
   uvicorn app.main:app --reload
   ```
...
```
[JudgeWorker] Redis 연결 실패: ...
```

**해결:**
1. Redis 서버 실행 확인
2. `.env` 파일의 Redis 설정 확인
3. Redis 포트가 6379인지 확인

### 문제 2: Judge0 API 연결 실패

```
[JudgeWorker] Judge0 API 호출 실패: ...
```

**해결:**
1. `.env` 파일의 Judge0 설정 확인
2. API 키가 올바른지 확인
3. RapidAPI 구독 상태 확인

### 문제 3: 큐에서 작업을 가져오지 못함

**증상:**
- Worker가 실행 중이지만 작업을 처리하지 않음
- 로그에 `[JudgeWorker] 작업 처리 시작` 메시지가 없음

**해결:**
1. `USE_REDIS_QUEUE=True` 설정 확인
2. FastAPI 서버와 Worker가 같은 Redis를 사용하는지 확인
3. 큐에 작업이 실제로 들어갔는지 확인

### 문제 4: 작업 실패 (status: failed)

**증상:**
```
[6c] Correctness 작업 추가 - task_id: correct_web-session-...
[6c] Correctness 작업 실패 - task_id: correct_web-session-...
```

**가능한 원인:**

1. **Memory Queue 프로세스 분리 문제** (가능성 높음)
   - `USE_REDIS_QUEUE=False`로 설정되어 Memory Queue를 사용 중
   - Memory Queue는 **프로세스 간 공유가 안 됨**
   - FastAPI 서버와 Judge0 Worker가 **별도 프로세스**에서 실행되면 큐를 공유하지 못함
   
   **해결 방법:**
   - `USE_REDIS_QUEUE=True`로 설정하여 Redis Queue 사용

2. **Judge0 API 호출 실패**
   - Worker가 작업을 가져옴
   - Judge0 API 호출 시도
   - API 호출 실패 (연결 실패, 타임아웃, 인증 실패 등)
   
   **확인 방법:**
   - Worker 로그에서 `[JudgeWorker] Judge0 API 호출 실패` 메시지 확인
   - 또는 `[JudgeWorker] 작업 처리 중 오류` 메시지 확인

3. **코드 실행 실패**
   - Worker가 작업을 가져옴
   - Judge0 API 호출 성공
   - 코드 실행 실패 (컴파일 에러, 런타임 에러 등)
   
   **확인 방법:**
   - Worker 로그에서 `[JudgeWorker] 작업 완료 - status: error` 확인
   - 결과 객체의 `error` 필드 확인

4. **Worker 처리 중 예외 발생**
   - Worker가 작업을 가져옴
   - 처리 중 예외 발생
   - `error_result` 생성하여 저장
   
   **확인 방법:**
   - Worker 로그에서 `[JudgeWorker] 작업 처리 중 오류` 메시지 확인
   - 예외 스택 트레이스 확인
...
1. **즉시 확인**
   ...
   - Judge0 API 연결 확인
     ...
     uv run python test_scripts/check_judge0_connection.py
     ```
   - 다시 테스트 실행하여 상세 로그 확인

2. **근본 해결**
   - Worker 로그의 에러 메시지에 따라 대응
   - Judge0 API 문제면 서버 상태 확인
   - 코드 실행 실패면 코드 검증 로직 추가
