    ...
    if parsed_json:
        try:
            ...
            if fallback_llm and formatted_messages:
                logger.info(
                    "[Structured Output Parser] Fallback: with_structured_output 사용"
                )
                import asyncio
                ...
                if asyncio.iscoroutinefunction(fallback_llm.ainvoke):
                    ...
                    raise ValueError(
                        f"비동기 fallback은 지원하지 않습니다. 파싱 실패: {e}"
                    )
                else:
        ...
        # JSON 추출 실패
        logger.warning(
            f"[Structured Output Parser] JSON 추출 실패 - content: {content[:200]}..."
        )
        if fallback_llm and formatted_messages:
            logger.info(
                "[Structured Output Parser] Fallback: with_structured_output 사용"
...
async def parse_structured_output_async(
    ...
    if parsed_json:
        try:
            ...
            if fallback_llm and formatted_messages:
                logger.info(
                    "[Structured Output Parser] Fallback: with_structured_output 사용 (비동기)"
                )
        ...
        logger.debug(
            f"[Structured Output Parser] JSON 추출 실패 (fallback 사용) - content: {content[:100]}..."
        )
        if fallback_llm and formatted_messages:
            logger.debug(
                "[Structured Output Parser] Fallback: with_structured_output 사용 (비동기)"
