    ...
    def wrap(self, chain: Runnable, name: str = "Chain") -> Runnable:
        ...
        async def logged_chain(
            ...
            if self.log_input:
                ...
                logger.log(self.log_level, f"[{name}] 입력 - {input_str}")
            ...
            try:
                ...
                if self.log_output:
                    ...
                    logger.log(self.log_level, f"[{name}] 출력 - {output_str}")

                # 실행 시간 로깅
                if self.log_timing:
                    logger.log(
                        self.log_level, f"[{name}] 실행 시간 - {elapsed_time:.3f}초"
                    )
        ...
        class LoggedRunnable(Runnable):
            def __init__(
                self, chain: Runnable, middleware: "LoggingMiddleware", name: str
            ):
                self.chain = chain
            ...
            def invoke(
                ...
                raise NotImplementedError(
                    "LoggingMiddleware는 비동기 호출만 지원합니다."
                )
