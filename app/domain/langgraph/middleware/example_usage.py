...
def example_intent_analyzer_with_middleware():
    ...
    base_chain = intent_analysis_prompt | structured_llm

    # 2. Middleware 적용
    rate_limiter = RateLimitingMiddleware(max_calls=15, period=60.0)
    retry_middleware = RetryMiddleware(
        max_retries=3, backoff_strategy="exponential", initial_delay=1.0
    )
    logging_middleware = LoggingMiddleware(
        log_level=logging.INFO, log_input=True, log_output=True, log_timing=True
    )
...
def example_writer_with_middleware():
    ...
    # 2. Middleware 적용
    rate_limiter = RateLimitingMiddleware(max_calls=15, period=60.0)
    retry_middleware = RetryMiddleware(max_retries=3, backoff_strategy="exponential")
    logging_middleware = LoggingMiddleware(name="Writer LLM")

    # 3. Middleware를 Chain에 적용
    chain_with_middleware = rate_limiter.wrap(
        retry_middleware.wrap(logging_middleware.wrap(base_chain, name="Writer LLM"))
    )
...
async def example_node_function_with_middleware(state):
    ...
    result = await chain_with_middleware.ainvoke({"human_message": human_message})
