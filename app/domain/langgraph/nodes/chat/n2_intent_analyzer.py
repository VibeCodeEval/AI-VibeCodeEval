...
class IntentAnalysisResult(BaseModel):
    ...
    guide_strategy: (
        ...
        None,
        description="가이드 전략 (SAFE인 경우). GENERATION=코드 제공(실행 가능한 구현 코드), FULL_CODE_ALLOWED=맥락 기반 풀 코드 생성",
    )
...
def get_llm():
    ...
    return create_gemini_llm(temperature=settings.LLM_TEMPERATURE_INTENT)
...
def quick_answer_detection(
    ...
    # [변경 2025-02] 코드 생성 허용으로 아래 패턴으로 인한 차단 로직은 비활성화됨. 추적용으로 정의만 유지.
    # 맥락 기반 판단이 필요한 패턴 (레거시: "전체 코드"/full/whole 포함 시 이전 대화에 코드 생성 없으면 BLOCKED)
    ...
    # [변경 2025-02] 코드 생성 전부 허용: "전체 코드"/full code/whole code 요청도 첫 턴부터 허용.
    # 기존: 이전 대화에 "코드 작성/생성" 맥락이 없으면 BLOCKED → 제거.
    # (레거시 로직 추적용 주석)
    # --- BEGIN: 레거시 - "전체 코드" 요청 맥락 기반 차단 (코드 생성 허용으로 비활성화) ---
