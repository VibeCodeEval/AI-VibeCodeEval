...
def create_normal_system_prompt(
    ...
    Args:
        ...
        guide_strategy: 가이드 전략 (SYNTAX_GUIDE | LOGIC_HINT | ROADMAP | GENERATION | FULL_CODE_ALLOWED)
            - GENERATION: 코드 제공 (실행 가능한 구현 코드)
            - FULL_CODE_ALLOWED: 맥락 기반 풀 코드 생성 (이전 대화 맥락 반영)
        keywords: 핵심 키워드
...
"""
    ...
    # 코드 생성 요청인 경우 추가 안내 (GENERATION / FULL_CODE_ALLOWED 시 실행 가능한 코드 제공)
    code_generation_section = ""
    if guide_strategy in ("GENERATION", "FULL_CODE_ALLOWED"):
...
def prepare_writer_input(state: MainGraphState) -> Dict[str, Any]:
    ...
    if not is_guardrail_failed:
        ...
        # intent_analyzer가 이미 코드 생성(GENERATION)으로 분류한 경우에도 코드 생성 경로 사용
        if guide_strategy_raw == "GENERATION":
            is_code_generation_request = True
