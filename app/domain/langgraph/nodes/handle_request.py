...
async def handle_request_load_state(state: MainGraphState) -> Dict[str, Any]:
    ...
    logger.info(
        f"[1. Handle Request] 진입 - session_id: {session_id}, 이전 턴: {current_turn}, 새 턴: {new_turn}"
    )
        ...
        if not state.get("problem_context") and spec_id:
            ...
            try:
                async with get_db_context() as db:
                    ...
                    logger.debug(
                        f"[1. Handle Request] DB에서 문제 정보 조회 성공 - spec_id: {spec_id}"
                    )
            ...
            logger.debug(
                f"[1. Handle Request] 문제 정보 추가 - spec_id: {spec_id}, problem_name: {problem_name}, source: {source}"
            )

        logger.info(
            f"[1. Handle Request] 완료 - session_id: {session_id}, 턴: {new_turn}"
        )
        return result

    except Exception as e:
        logger.error(
            f"[1. Handle Request] 오류 - session_id: {session_id}, error: {str(e)}",
            exc_info=True,
        )
        raise
