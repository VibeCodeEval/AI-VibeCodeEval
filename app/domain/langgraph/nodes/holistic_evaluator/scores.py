...
async def aggregate_turn_scores(state: MainGraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not turn_scores:
            logger.warning(
                f"[6b. Aggregate Turn Scores] 턴 점수 없음 - session_id: {session_id}"
            )
            return {
        ...
        if not all_scores:
            logger.warning(
                f"[6b. Aggregate Turn Scores] 유효한 점수 없음 - session_id: {session_id}"
            )
            return {
                "aggregate_turn_score": None,
                "updated_at": datetime.utcnow().isoformat(),
        ...
        logger.info(
            f"[6b. Aggregate Turn Scores] 완료 - session_id: {session_id}, 턴 개수: {len(all_scores)}, 평균: {avg_score:.2f}"
        )
        ...
        logger.error(
            f"[6b. Aggregate Turn Scores] 오류 - session_id: {session_id}, error: {str(e)}",
            exc_info=True,
        )
        return {
            "aggregate_turn_score": None,
...
async def aggregate_final_scores(state: MainGraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[7. Aggregate Final Scores]   - Holistic Flow Score: {holistic_flow_score}"
        )
        logger.info(
            f"[7. Aggregate Final Scores]   - Aggregate Turn Score: {aggregate_turn_score}"
        )
        logger.info(
            f"[7. Aggregate Final Scores]   - Code Performance Score: {code_performance_score}"
        )
        logger.info(
        ...
        logger.info(
            f"[7. Aggregate Final Scores] Prompt Score: {prompt_score:.2f} (가중치: 40%)"
        )
        logger.info(
            f"[7. Aggregate Final Scores] Performance Score: {perf_score:.2f} (가중치: 30%)"
        )
        logger.info(
            f"[7. Aggregate Final Scores] Correctness Score: {correctness_score:.2f} (가중치: 30%)"
        )
        ...
        logger.error(
            f"[7. Aggregate Final Scores] 오류 - session_id: {session_id}, error: {str(e)}",
            exc_info=True,
        )
        return {
            "final_scores": None,
