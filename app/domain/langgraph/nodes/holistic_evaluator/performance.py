...
from app.domain.langgraph.nodes.holistic_evaluator.langsmith_utils import (
    TRACE_NAME_CODE_PERFORMANCE, should_enable_langsmith,
...
async def _eval_code_performance_impl(state: MainGraphState) -> Dict[str, Any]:
    ...
    if not code_content:
        logger.warning(
            f"[6c. Eval Code Performance] 코드 없음 - session_id: {session_id}"
        )
        return {
    ...
    logger.info(
        f"[6c. Eval Code Performance] 코드 평가 시작 - session_id: {session_id}, 코드 길이: {len(code_content)}"
    )
    ...
    try:
        ...
        while time.time() - start_time < max_wait:
            ...
            if status == "completed":
                ...
                if result and result.status == "success":
                    ...
                    logger.warning(
                        f"[6c] Judge0 실행 실패 - task_id: {task_id}, error: {error_msg}"
                    )
                    # LLM 기반 평가로 폴백
                    break
        ...
        logger.warning(
            f"[6c] Judge0 타임아웃 또는 실패 - task_id: {task_id}, LLM 기반 평가로 폴백"
        )

    except Exception as e:
        logger.warning(
            f"[6c] Judge0 큐 시스템 오류 - session_id: {session_id}, error: {str(e)}, LLM 기반 평가로 폴백"
        )
    ...
    def process_performance_output_with_response(
        inputs: Dict[str, Any],
    ) -> Dict[str, Any]:
        """출력 처리 (LLM 응답 객체 포함)"""
    ...
    try:
        ...
        if tokens:
            ...
            logger.debug(
                f"[6c. Eval Code Performance] 토큰 사용량 - prompt: {tokens.get('prompt_tokens')}, completion: {tokens.get('completion_tokens')}, total: {tokens.get('total_tokens')}"
            )
        else:
            logger.warning(
                f"[6c. Eval Code Performance] 토큰 사용량 추출 실패 - raw_response 타입: {type(raw_response)}"
            )
        ...
        logger.info(
            f"[6c. Eval Code Performance] 완료 - session_id: {session_id}, score: {result['code_performance_score']}"
        )

        # LangSmith 추적 정보 로깅
        if should_enable_langsmith(state):
            logger.debug(
                f"[LangSmith] 6c 노드 추적 활성화 - session_id: {session_id}, 코드 길이: {len(code_content)}"
            )
        ...
        logger.error(
            f"[6c. Eval Code Performance] 오류 - session_id: {session_id}, error: {str(e)}",
            exc_info=True,
        )
        return {
            "code_performance_score": None,
