...
async def _eval_code_correctness_impl(state: MainGraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"[6d] Judge0 작업 추가 - task_id: {task_id}, test_cases: {len(test_cases)}")
            ...
            if status == "completed":
                ...
                if result:
                    if result.status == "success" and test_cases:
                        ...
                        logger.warning(f"[6d] Judge0 실행 실패 - task_id: {task_id}, error: {error_msg}")
                        # LLM 기반 평가로 폴백
                        break
        ...
        logger.warning(f"[6d] Judge0 타임아웃 또는 실패 - task_id: {task_id}, LLM 기반 평가로 폴백")
        
    except Exception as e:
        logger.warning(f"[6d] Judge0 큐 시스템 오류 - session_id: {session_id}, error: {str(e)}, LLM 기반 평가로 폴백")
    ...
    def process_correctness_output_with_response(inputs: Dict[str, Any]) -> Dict[str, Any]:
        """출력 처리 (LLM 응답 객체 포함)"""
    ...
    try:
        ...
        if tokens:
            ...
            logger.debug(f"[6d. Eval Code Correctness] 토큰 사용량 - prompt: {tokens.get('prompt_tokens')}, completion: {tokens.get('completion_tokens')}, total: {tokens.get('total_tokens')}")
        else:
            logger.warning(f"[6d. Eval Code Correctness] 토큰 사용량 추출 실패 - raw_response 타입: {type(raw_response)}")
        ...
        logger.info(f"[6d. Eval Code Correctness] 완료 - session_id: {session_id}, score: {result['code_correctness_score']}")
        
        # LangSmith 추적 정보 로깅
        if should_enable_langsmith(state):
            logger.debug(f"[LangSmith] 6d 노드 추적 활성화 - session_id: {session_id}, 코드 길이: {len(code_content)}")
