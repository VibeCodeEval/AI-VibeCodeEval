...
from app.domain.langgraph.nodes.eval.utils import get_llm
...
위 로그를 분석하여 Chaining 전략 점수를 평가하세요."""
        ...
        def process_holistic_output_with_response(
            ...
            # V2.3: 1~5 정수 → 0~100 환산 (Python 위임)
            score_0_100 = likert_to_final(int(result.overall_flow_score))

            processed = {
                "holistic_flow_score": score_0_100,
                "holistic_flow_analysis": result.analysis,
        ...
        try:
            ...
            # V2.3: 1~5 정수 → 0~100 환산 (Python 위임)
            score_0_100 = likert_to_final(int(structured_result.overall_flow_score))
            ...
            logger.info(
                f"[6a. Eval Holistic Flow] 구조화된 출력 파싱 완료 - Likert {structured_result.overall_flow_score} → {score_0_100}점"
            )
