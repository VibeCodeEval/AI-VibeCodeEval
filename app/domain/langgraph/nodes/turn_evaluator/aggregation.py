...
async def aggregate_turn_log(state: EvalTurnState) -> Dict[str, Any]:
    ...
    for eval_key, eval_data in eval_results.items():
        if isinstance(eval_data, dict):
            ...
            if rubrics:
                ...
                logger.debug(
                    f"[4.4 턴 로그 집계] {eval_key} 가중치 적용 점수: {weighted_score:.2f} (의도: {intent_upper})"
                )
            else:
                ...
                logger.warning(
                    f"[4.4 턴 로그 집계] {eval_key} 루브릭 없음 - 기존 score 사용: {score:.2f}"
                )
    ...
    if is_guardrail_failed:
        ...
        logger.info(
            f"[4.4 턴 로그 집계] 가중치 적용 완료 - 의도: {intent_upper}, 최종 점수: {turn_score:.2f}"
        )
