...
class JudgeWorker:
    ...
    async def _worker_loop(self):
        ...
        while self.running:
            ...
            try:
                ...
                logger.error(
                    f"[JudgeWorker] 작업 처리 중 오류: {str(e)}", exc_info=True
                )
    ...
    async def _execute_task(self, task: JudgeTask) -> JudgeResult:
        ...
        try:
            ...
            logger.info(
                f"[JudgeWorker]   - 코드 라인 수: {len(task.code.split(chr(10)))}줄"
            )
            ...
            if task.test_cases:
                for idx, tc in enumerate(task.test_cases, 1):
                    ...
                    logger.info(
                        f"[JudgeWorker]   - TC {idx}: input 길이={len(tc_input)} 문자, expected={tc_expected}"
                    )
                    if len(tc_input) <= 200:
                        logger.info(f"[JudgeWorker]     input 내용: {tc_input}")
                    else:
                        logger.info(
                            f"[JudgeWorker]     input 내용 (처음 200자): {tc_input[:200]}..."
                        )
            else:
                logger.warning(
                    f"[JudgeWorker]   ⚠️ 테스트 케이스 없음 - stdin이 비어있을 수 있음"
                )
            ...
            if task.test_cases:
                ...
                test_case_results = await self.judge0_client.execute_test_cases(
                    ...
                    test_cases=[
                        {
                            "input": (
                                tc.get("input", "") if isinstance(tc, dict) else str(tc)
                            ),
                            "expected": (
                                tc.get("expected") if isinstance(tc, dict) else None
                            ),
                        }
                ...
                if status_id == 3:  # Accepted
                    ...
                    status = "error"
                    error = result.get("stderr") or result.get("status", {}).get(
