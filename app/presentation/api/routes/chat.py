    ...
    try:
        ...
        async def calculate_user_tokens():
            ...
            try:
                ...
                logger.warning(
                    f"[SendMessages] 사용자 메시지 토큰 계산 실패: {str(e)}, 대략적 추정 사용"
                )
        ...
        result, user_message_tokens = await asyncio.gather(
            asyncio.wait_for(langgraph_task, timeout=120.0), user_tokens_task
        )
        ...
        if request.turnId > 1:
            try:
                # Redis에서 조회 시도
                previous_tokens_redis = await redis_client.get(
                    f"session_token:{session.id}"
                )
                if previous_tokens_redis:
                    ...
                    await redis_client.set(
                        f"session_token:{session.id}", str(previous_tokens)
                    )
            except Exception as e:
                logger.warning(
                    f"[SendMessages] 이전 토큰 조회 실패: {str(e)}, 0으로 시작"
                )
                previous_tokens = 0
        else:
            # 첫 Turn이면 이전 토큰은 0
            logger.debug(
                f"[SendMessages] 첫 Turn (turnId={request.turnId}), previous_tokens=0"
            )
...
async def get_problem_info(
    ...
    try:
        from app.domain.langgraph.utils.problem_info import \
            get_problem_info as get_problem_info_async
...
async def get_token_usage(
    ...
    try:
        ...
        from app.domain.langgraph.utils.token_tracking import (
            format_tokens_for_core, get_token_summary)
        ...
        logger.info(
            f"[Token Usage API] 조회 완료 - session_id: {session_id}, chat: {chat_tokens}, eval: {eval_tokens}"
        )
...
async def save_chat_message(
    request: SaveChatMessageRequest, db: AsyncSession = Depends(get_db)
) -> SaveChatMessageResponse:
    """
    메시지 저장 (Spring Boot용)
    ...
    try:
        ...
        return SaveChatMessageResponse(
            session_id=0, message_id=0, success=False, error_message=str(e)
        )
    except Exception as e:
        # 기타 오류
