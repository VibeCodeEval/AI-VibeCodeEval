...
class ChatResponse(BaseModel):
    ...
    chat_tokens: Optional[Dict[str, int]] = Field(
        None, description="채팅 검사 토큰 사용량 (Intent Analyzer, Writer LLM)"
    )
    eval_tokens: Optional[Dict[str, int]] = Field(
        None, description="평가 토큰 사용량 (Turn Evaluator, Holistic Evaluator)"
    )
    total_tokens: Optional[Dict[str, int]] = Field(
        None, description="전체 토큰 사용량 (chat + eval 합계, Core 전달용)"
    )
...
class EvaluationFeedback(BaseModel):
    ...
    holistic_flow_analysis: Optional[str] = Field(
        None,
        description="체이닝 전략에 대한 상세 분석 (문제 분해, 피드백 수용성, 주도성, 전략적 탐색)",
    )
...
class SubmitResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "turn_scores": {"1": {"turn_score": 82.0}, "2": {"turn_score": 88.0}},
                "error": False,
                "error_message": None,
            }
        }
    ...
    feedback: Optional[EvaluationFeedback] = Field(
        None, description="평가 피드백 (체이닝 전략 분석 등)"
    )
    chat_tokens: Optional[Dict[str, int]] = Field(
        None, description="채팅 검사 토큰 사용량 (Intent Analyzer, Writer LLM)"
    )
...
class SubmitCodeResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"submissionId": 88001, "status": "successed"}}
    )
...
class ChatMessagesRequest(BaseModel):
    ...
    participantId: int = Field(
        ..., description="참가자 ID (participants.id)", alias="participantId"
    )
...
class AIMessageInfo(BaseModel):
    ...
    tokenCount: int = Field(
        ...,
        description="현재 턴 대화 토큰 (사용자 메시지 + AI 응답)",
        alias="tokenCount",
    )
    totalToken: int = Field(
        ..., description="한 세션의 모든 대화 token_count의 합", alias="totalToken"
    )
