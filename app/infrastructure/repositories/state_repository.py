...
class StateRepository:
    ...
    async def save_state(
        self, session_id: str, state: dict, ttl_seconds: Optional[int] = None
    ) -> bool:
        """그래프 상태 저장"""
        # messages 직렬화
        state_copy = {**state}
        ...
        return await self.redis.save_graph_state(
            session_id, state_with_meta, ttl_seconds
        )
    ...
    async def update_state_field(self, session_id: str, field: str, value: Any) -> bool:
        """그래프 상태의 특정 필드 업데이트"""
    ...
    async def save_checkpoint(
        ...
        return await self.redis.save_checkpoint(
            session_id, checkpoint_id, checkpoint_data, ttl_seconds
        )
    ...
    async def get_turn_data(self, session_id: str, turn: int) -> Optional[dict]:
        """턴 데이터 조회"""
        key = self._turn_key(session_id, turn)
    ...
    async def save_turn_scores(self, session_id: str, turn: int, scores: dict) -> bool:
        """턴별 점수 저장"""
    ...
    async def save_final_scores(self, session_id: str, final_scores: dict) -> bool:
        """최종 점수 저장"""
        key = self._score_key(session_id)
        existing = await self.redis.get_json(key) or {"turns": {}}
        ...
        return await self.redis.set_json(
            key, existing, settings.CHECKPOINT_TTL_SECONDS * 2
        )
    ...
    async def save_memory_summary(
        self, session_id: str, summary: str, context: Optional[dict] = None
    ) -> bool:
        """메모리 요약 저장"""
