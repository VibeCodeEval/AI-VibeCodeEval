...
from app.infrastructure.persistence.models.enums import (SubmissionStatusEnum,
                                                         VerdictEnum)
from app.infrastructure.persistence.models.submissions import (Score,
                                                               Submission,
                                                               SubmissionRun)
...
class SubmissionRepository:
    ...
    async def update_submission_status(
        self, submission_id: int, status: SubmissionStatusEnum
    ) -> Optional[Submission]:
        """제출 상태 업데이트"""
    ...
    async def get_submission_runs(self, submission_id: int) -> List[SubmissionRun]:
        """제출 실행 결과 목록 조회"""
        query = (
            select(SubmissionRun)
            .where(SubmissionRun.submission_id == submission_id)
