...
from app.infrastructure.persistence.models.exams import (Exam, ExamParticipant,
                                                         ExamStatistics)
...
class ExamRepository:
    ...
    async def get_problem_spec(self, spec_id: int) -> Optional[ProblemSpec]:
        ...
        Args:
            spec_id: ``problem_specs.spec_id`` (PK). API의 specId와 동일. ``problem_id`` 아님.
        """
        query = select(ProblemSpec).where(ProblemSpec.spec_id == spec_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def get_problem_spec_with_problem(
        self, spec_id: int
    ) -> Optional[ProblemSpec]:
        """
        ``problem_specs.spec_id == spec_id`` 인 행 + 연결된 ``problems`` 행(selectinload).

        Args:
            spec_id: ``problem_specs`` 테이블 PK (API specId).
        """
        query = (
            select(ProblemSpec)
            .options(selectinload(ProblemSpec.problem))
            .where(ProblemSpec.spec_id == spec_id)
        )
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def update_participant_state(
        self, exam_id: int, participant_id: int, state: str
    ) -> Optional[ExamParticipant]:
        """참가자 상태 업데이트"""
    ...
    async def update_participant_tokens(
        self, exam_id: int, participant_id: int, additional_tokens: int
    ) -> Optional[ExamParticipant]:
        """참가자 토큰 사용량 업데이트"""
        participant = await self.get_exam_participant(exam_id, participant_id)
        if participant:
    ...
    async def check_token_limit(
        self, exam_id: int, participant_id: int, required_tokens: int = 0
    ) -> bool:
        """토큰 한도 확인"""
        participant = await self.get_exam_participant(exam_id, participant_id)
