...
from app.infrastructure.persistence.models.enums import (DifficultyEnum,
                                                         ProblemStatusEnum)
...
class Problem(Base):
    ...
    difficulty: Mapped[DifficultyEnum] = mapped_column(
        Enum(DifficultyEnum, name="ai_vibe_coding_test.difficulty_enum"), nullable=False
    )
    ...
    specs: Mapped[List["ProblemSpec"]] = relationship(
        "ProblemSpec", back_populates="problem", foreign_keys="ProblemSpec.problem_id"
    )
    current_spec: Mapped[Optional["ProblemSpec"]] = relationship(
        "ProblemSpec", foreign_keys=[current_spec_id], post_update=True
    )
...
class ProblemSpec(Base):
    ...
    problem: Mapped["Problem"] = relationship(
        "Problem", back_populates="specs", foreign_keys=[problem_id]
    )
...
class ProblemSet(Base):
    ...
    items: Mapped[List["ProblemSetItem"]] = relationship(
        "ProblemSetItem", back_populates="problem_set"
    )
...
class ProblemSetItem(Base):
    ...
    problem_set_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("problem_sets.id"), nullable=False
    )
    problem_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("problems.id"), nullable=False
    )
    ...
    problem_set: Mapped["ProblemSet"] = relationship(
        "ProblemSet", back_populates="items"
    )
