...
class PromptSession(Base):
    ...
    messages: Mapped[List["PromptMessage"]] = relationship(
        "PromptMessage", back_populates="session", order_by="PromptMessage.turn"
    )
...
class PromptMessage(Base):
    ...
    session_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("prompt_sessions.id"), nullable=False
    )
    ...
    session: Mapped["PromptSession"] = relationship(
        "PromptSession", back_populates="messages"
    )
...
class PromptEvaluation(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    session_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("prompt_sessions.id", ondelete="CASCADE"), nullable=False
    )
    turn: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True  # NULL이면 세션 전체 평가 (HOLISTIC_FLOW)
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "turn",
            "evaluation_type",
            name="prompt_evaluations_session_turn_type_unique",
        ),
    )

    # Relationships
    session: Mapped["PromptSession"] = relationship(
        "PromptSession", foreign_keys=[session_id], back_populates="evaluations"
    )
