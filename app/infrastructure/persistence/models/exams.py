...
class Exam(Base):
    ...
    problem_set_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey("problem_sets.id"), nullable=True
    )
    ...
    participants: Mapped[List["ExamParticipant"]] = relationship(
        "ExamParticipant", back_populates="exam"
    )
    statistics: Mapped[Optional["ExamStatistics"]] = relationship(
        "ExamStatistics", back_populates="exam", uselist=False
    )
...
class ExamStatistics(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    exam_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("exams.id"), nullable=False, unique=True
    )
