...
from sqlalchemy import (CHAR, BigInteger, DateTime, Enum, ForeignKey, Integer,
                        Numeric, String, Text)
...
class Submission(Base):
    ...
    runs: Mapped[List["SubmissionRun"]] = relationship(
        "SubmissionRun", back_populates="submission"
    )
...
class SubmissionRun(Base):
    ...
    # Relationships
    submission: Mapped["Submission"] = relationship("Submission", back_populates="runs")
...
class Score(Base):
    ...
    submission_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("submissions.id"), primary_key=True, nullable=False
    )
    prompt_score: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(5, 2), nullable=True
    )
    perf_score: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), nullable=True)
    ...
    submission: Mapped["Submission"] = relationship(
        "Submission", back_populates="score"
    )
